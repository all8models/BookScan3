...
### 1.1 현상 및 분석
- 기존 [ARCHITECTURE.md](../ARCHITECTURE.md)는 530여 줄에 달하는 arc42 기반 표준 아키텍처 문서로 작성되어 있습니다.
...
- 그러나 실제 코드베이스에는:
  ...
  4. 곡면 보정은 Core ML 3단계가 아니라 단일 수평 삼각함수 워프 커널([CylindricalDewarpService.swift](../BookScan3/Services/CylindricalDewarpService.swift))만 동작합니다.
...
- [Services](../BookScan3/Services) 계층의 핵심 클래스들([CameraService](../BookScan3/Services/CameraService.swift), [StorageManager](../BookScan3/Services/StorageManager.swift), [ImageProcessor](../BookScan3/Services/ImageProcessor.swift), [BookSpreadDetector](../BookScan3/Services/BookSpreadDetector.swift), [PCTransferServer](../BookScan3/Services/PCTransferServer.swift))에 대응하는 **Swift Protocol 추상화가 단 하나도 없습니다**.
...
- [LibraryViewModel.swift](../BookScan3/ViewModels/LibraryViewModel.swift)는 앱의 모든 비즈니스 작업을 단 하나의 `@Published private(set) var busy = false` 플래그로 직렬화하고 있습니다.
...
- [ImageProcessor.context](../BookScan3/Services/ImageProcessor.swift#L63)가 전역 정적 변수(`static let context = CIContext(...)`)로 단 하나만 생성되어 카메라 분석 큐, 캡처 저장 큐, 썸네일 렌더링 큐에서 무제한 동시 접근합니다. Thread-safe하더라도 GPU 렌더 큐 병목을 유발합니다.
...
- [StorageManager.swift](../BookScan3/Services/StorageManager.swift)는 서재 전체 데이터를 `library.json` 단일 파일로 관리합니다.
...
### 4.3 쿼리 및 검색 인덱싱 부재
- [LibraryView.swift](../BookScan3/Views/LibraryView.swift#L16-L19)의 도서 검색은 인메모리에서 모든 책과 모든 페이지 텍스트를 선형 전수 탐색(`localizedCaseInsensitiveContains`)합니다. CoreData/SQLite와 같은 인덱스 기반 검색 엔진이 없어 서재 규모 확장에 취약합니다.
...
- [BookSpreadDetector.swift](../BookScan3/Services/BookSpreadDetector.swift)는 양면 검출과 접힘선(Seam) 추정을 수십 개의 고정 상수에 의존하고 있습니다:
...
- [CylindricalDewarpService.swift](../BookScan3/Services/CylindricalDewarpService.swift#L6-L15)에서 `CIWarpKernel(source: ...)` 런타임 문자열 컴파일 방식을 사용합니다.
...
- [PCTransferServer.swift](../BookScan3/Services/PCTransferServer.swift#L5-L16)는 HTTP 요청을 `\r\n` 및 공백 분할 방식으로 파싱합니다.
...
- [PCTransferServer.swift](../BookScan3/Services/PCTransferServer.swift#L124)의 IP 주소 조회 로직:
  ```swift
...
- [BookScan3Tests](../BookScan3Tests)를 분석한 결과:
...
- 최상위 [BookScan3App.swift](../BookScan3/BookScan3App.swift)에서 주입된 단일 `LibraryViewModel`이 모든 하위 뷰([LibraryView](../BookScan3/Views/LibraryView.swift), [BookDetailView](../BookScan3/Views/BookDetailView.swift), [CameraScannerView](../BookScan3/Views/CameraScannerView.swift), [SpreadReviewView](../BookScan3/Views/SpreadReviewView.swift), [PageDetailView](../BookScan3/Views/PageDetailView.swift))로 전역 전달됩니다.
